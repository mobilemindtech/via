...
import io.micro.router.types.*
import scala.annotation.tailrec
import scala.collection.mutable
import scala.util.matching.Regex

import Query.*, RouteQuery.*, QueryType.*, RegexType.*
...
object RouteChain:
  ...
  private case class RouteChainInfo(
      route: Route,
      matcher: RouteMatcher,
  ...
  private def doChain(
    ...
    @tailrec
    def find(rts: List[Route]): Option[RouteChainInfo] =
      rts match
        case x :: xs =>
          ...
          then find(xs)
            ...
            then find(xs)
              ...
              x.params match
                case Nil =>
                  Some(RouteChainInfo(x, mkRouteMatcher(x, Nil), Params(Nil), query))
                case pathParams =>
                  ...
                  Some(RouteChainInfo(x, matcher, Params(pars), query))
