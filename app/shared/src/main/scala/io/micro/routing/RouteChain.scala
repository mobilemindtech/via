...
import io.micro.routing.Query._
import io.micro.routing.RouteQuery._
...
object RouteChain:
          ...
          then
            ...
            x.params match
              ...
              case pathParams =>
                ...
                Some(RouteInfo(x, matcher, Params(pars), query))
  ...
  private def toQueryIntOpt(v: String): Option[QueryInt] = v.toIntOption.map(QueryInt.apply)
  
  private def toQueryLongOpt(v: String): Option[QueryLong] = v.toLongOption.map(QueryLong.apply)
  
  private def toQueryStrOpt(v: String): Option[QueryStr] = Option.when(v.nonEmpty)(v).map(QueryStr.apply)
  ...
  private def toQueryBoolOpt(v: String): Option[QueryBool] = v.toBooleanOption.map(QueryBool.apply)
  
  private def toQueryList[T](v: Option[String], f: String => Option[T]): List[Option[T]] = v match
    case Some(s) => s.split(",").toList.map(f)
    case None => Nil
  
  private def toQueryTypeOrError[T <: QueryType](v: Option[String], f: String => Option[T]): QueryType = v match
  ...
  private def toQueryList(qs: Map[String, String], typ: RouteQueryListType): QueryType = typ match
    case RouteQueryInt(name) =>
      toQueryList(findQueryValue(qs, name), toQueryIntOpt).map(unwrapRouteTypeOpt) |> QueryList.apply
    case RouteQueryLong(name) =>
      toQueryList(findQueryValue(qs, name), toQueryLongOpt).map(unwrapRouteTypeOpt) |> QueryList.apply
    case RouteQueryStr(name) =>
      toQueryList(findQueryValue(qs, name), toQueryStrOpt).map(unwrapRouteTypeOpt) |> QueryList.apply
    case RouteQueryRegex(name, pattern) =>
      toQueryList(findQueryValue(qs, name), toQueryRegexOpt(pattern)).map(unwrapRouteTypeOpt) |> QueryList.apply
    case _ => QueryInvalid()
  
  private def toQueryOpt(qs: Map[String, String], typ: RouteQueryOptType): QueryOption = typ match
    case RouteQueryInt(name) =>
    ...
    case RouteQueryRegex(name, pattern) =>
      findQueryValue(qs, name).flatMap(toQueryRegexOpt(pattern)) |> QueryOption.apply
    case RouteQueryLong(name) =>
  ...
  private def getQueryName(typ: RouteQueryListType | RouteQueryOptType): String = typ match
    ...
    case typ => throw RouteException(s"invalid route query type to get query name: ${typ}")
